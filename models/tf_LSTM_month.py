...
pruned_data = scaled_dataset[1000:]
print(len(X))
print(len(y))

X_train = np.array(X[1000:1225])
y_train = np.array(y[1000:1225])

X_test = np.array(X[1225:])
y_test = np.array(y[1225:])
...
class StockPredictionRNN(object):
    def __init__(self, learning_rate=learning_rate, batch_size=7, hidden_layer_size=hidden_layer_size, number_of_layers=1,
                 dropout=True, dropout_rate=dropout_rate, number_of_classes=1, gradient_clip_margin=gradient_clip_margin, window_size=7):
...
for i in range(1000, 1253):
    if i >= 1225:
        test_results.append(tests_new[i-1225])
    else:
